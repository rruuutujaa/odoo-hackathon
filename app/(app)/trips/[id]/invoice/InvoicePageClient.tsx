...
import { updateInvoiceStatus } from "@/lib/actions/expenses"; // We will create this
...
export default function InvoicePageClient({ trip, summary, id }: { trip: any, summary: any, id: string }) {
  ...
  const handleExportPDF = () => {
    ...
    doc.text(`INVOICE: ${trip.title}`, 20, 20);
    doc.setFontSize(10);
    doc.text(`Traveler: ${trip.user.firstName} ${trip.user.lastName}`, 20, 30);
    doc.text(`Dates: ${format(new Date(trip.startDate), "MMM dd")} - ${format(new Date(trip.endDate), "MMM dd, yyyy")}`, 20, 35);
    
    const tableData = trip.expenses.map((exp: any) => [
      exp.description,
      exp.category,
      exp.quantity,
      `$${exp.unitCost}`,
    ...
    doc.text(`Subtotal: $${summary.subtotal}`, 150, (doc as any).lastAutoTable.finalY + 10);
    doc.text(`Grand Total: $${summary.grandTotal}`, 150, (doc as any).lastAutoTable.finalY + 20);
    
    doc.save(`Invoice-${trip.title.replace(/\s+/g, '-')}.pdf`);
    toast({ title: "Export Successful", description: "Your invoice has been downloaded as PDF." });
  };
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
            <Button variant="outline" onClick={handleExportPDF} className="rounded-xl border-muted-foreground/20 font-bold hover:bg-muted/50 h-12">
              <Download className="mr-2 h-4 w-4" /> Export PDF
            </Button>
            {trip.invoice?.paymentStatus !== "PAID" && (
              <Button onClick={handleMarkPaid} disabled={isUpdating} className="bg-[#1A1F3C] hover:bg-black text-white rounded-xl h-12 px-6 font-bold">

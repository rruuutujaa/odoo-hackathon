    ...
    <div className="space-y-24 md:space-y-40">
      ...
      {featuredTrip && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 border border-foreground/5 bg-card overflow-hidden luxury-shadow">
          <div className="lg:col-span-8 relative aspect-[16/10] lg:aspect-auto h-64 lg:h-full overflow-hidden">
             <img 
          ...
          <div className="lg:col-span-4 p-8 md:p-16 flex flex-col justify-between space-y-12 lg:space-y-20 bg-background border-l border-foreground/5">
             <div className="space-y-6 md:space-y-8">
                <span className="text-[10px] font-black uppercase tracking-[0.5em] text-primary">Priority Loop</span>
                <h2 className="text-4xl md:text-7xl font-display leading-[0.9] tracking-tighter">{featuredTrip.title}</h2>
                <p className="text-lg md:text-xl text-muted-foreground font-medium leading-relaxed italic border-l border-primary/30 pl-6 md:pl-8">
      ...
      <div className="space-y-12 md:space-y-20">
         <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-foreground/10 pb-8 md:pb-10 gap-4">
            <h3 className="text-3xl md:text-5xl font-display tracking-tight">Archived Records.</h3>
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-1px bg-foreground/5">
            {trips.slice(1, 4).map((trip) => (
              <div key={trip.id} className="bg-background p-8 md:p-12 space-y-8 md:space-y-10 group cursor-pointer hover:bg-muted/30 transition-colors">
                 ...
                 <div className="space-y-4">
                    <h4 className="text-2xl md:text-3xl font-display leading-none group-hover:text-primary transition-colors">{trip.title}</h4>
...
export default async function DashboardPage() {
  ...
  return (
    <div className="page-transition min-h-screen">
      ...
      <header className="py-12 px-6 md:px-12 lg:section-padding grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-end border-b border-foreground/5 bg-background">
        <div className="lg:col-span-7 space-y-8 lg:space-y-12">
          ...
          <div className="max-w-md space-y-6 lg:space-y-10">
            <p className="text-lg md:text-2xl text-muted-foreground leading-snug font-medium">
            ...
            <Button asChild size="lg" className="w-full sm:w-fit rounded-none bg-foreground text-background hover:bg-primary hover:text-white px-12 h-16 font-black uppercase tracking-[0.3em] text-xs transition-all shadow-none">
